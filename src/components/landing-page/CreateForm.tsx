...
export default function CreateForm() {
  ...
  const generateHabitTracker = async (e: React.FormEvent) => {
    e.preventDefault();

    router.push(`/preview?habitName=${habitName}&startDate=${startDate?.toISOString()}&endDate=${endDate?.toISOString()}&phrase=${useCustomPhrase ? customPhrase : "La disciplina es el puente entre metas y logros"}&signature=${includeSignature ? signature : ""}`);
  };

  return (
    <form onSubmit={generateHabitTracker} className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-2">
          ...
          <Label htmlFor="include-signature" className="text-white">
            ...
            <span className="text-zinc-400 text-sm ml-2">(Recomendado)</span>
          </Label>
        </div>
